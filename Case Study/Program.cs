    ...
    class Program
    {
        ...
        static void AdminLogin()
        {
            ...
            if (adminFromDb.Authenticate(password))
            {
                ...
                ShowAdminMenu(); // show dashboard after successful login
            }
            else
            {
            ...
            if (int.TryParse(Console.ReadLine(), out int id))
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = @"UPDATE Vehicle 
                             SET Make = @Make, Model = @Model, Type = @Type, Year = @Year, 
                                 FuelType = @FuelType, SeatingCapacity = @SeatingCapacity, 
                                 DailyRent = @DailyRent, AvailabilityStatus = @AvailabilityStatus 
                    ...
                    cmd.Parameters.AddWithValue("@Type", string.IsNullOrWhiteSpace(type) ? vehicle.Type : type);
                    cmd.Parameters.AddWithValue("@Year", string.IsNullOrWhiteSpace(yearInput) ? vehicle.Year : int.Parse(yearInput));

                    cmd.Parameters.AddWithValue("@RentPerDay", string.IsNullOrWhiteSpace(DailyRent) ? vehicle.DailyRate : decimal.Parse(DailyRent));
                    cmd.Parameters.AddWithValue("@AvailabilityStatus", string.IsNullOrWhiteSpace(status) ? vehicle.IsAvailable : status);
        ...
        static void LoadReservations()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                while (reader.Read())
                {
                    ...
                    res.TotalCost = Convert.ToDouble(reader["TotalCost"]);
